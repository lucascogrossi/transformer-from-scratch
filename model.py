...
class LayerNormalization(nn.Module):
    def __init__(self, epsilon: float = 1e-6) -> None:
        ...
        self.gamma = nn.Parameter(torch.ones(1)) # weight
...
class FeedForward(nn.Module):
    def __init__(self, d_model: int, d_ff: int, dropout: float) -> None:
        super().__init__()
        self.linear_1 = nn.Linear(d_model, d_ff) # weight (W1) and bias (B1)
        self.dropout = nn.Dropout(dropout)
        self.linear_2 = nn.Linear(d_ff, d_model) # weight (W2) and bias (B2)
...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int=512, N: int=6, h: int=8, dropout: float=0.1, d_ff=2048) -> Transformer:
    ...
    for _ in range(N):
        ...
        encoder = EncoderBlock(encoder_self_attention, feed_forward, dropout)
    ...
    for _ in range(N):
        ...
        feed_forward = FeedForward(d_model, d_ff, dropout)
        decoder = DecoderBlock(decoder_self_attention, decoder_cross_attention, feed_forward, dropout)
